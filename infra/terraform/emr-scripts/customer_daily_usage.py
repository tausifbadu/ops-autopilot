...
def main():
    ...
    customer_meter = spark.read.parquet(f"{input_path}customer_meter_mapping/")

    usage = meter_usage.withColumn("usage_date", F.to_date(F.col("timestamp")))
    joined = usage.join(customer_meter, on="meter_id", how="inner")

    daily = (
        joined.groupBy("customer_id", "usage_date")
        .agg(
